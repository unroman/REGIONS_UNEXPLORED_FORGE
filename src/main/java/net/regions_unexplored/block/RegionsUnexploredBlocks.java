...
public class RegionsUnexploredBlocks {
    ...
    private static JoshuaLeavesBlock joshuaLeaves(MaterialColor colour) {
        return new JoshuaLeavesBlock(BlockBehaviour.Properties.of(Material.LEAVES, colour).strength(0.2F).randomTicks().sound(SoundType.GRASS).noOcclusion().noCollission().offsetType((state) -> {return state.getValue(JoshuaLeavesBlock.NATURAL) == true ? BlockBehaviour.OffsetType.NONE : BlockBehaviour.OffsetType.XZ;}));
    }

...
public class AlphaGrassBlock extends SpreadingAlphaDirtBlock implements BonemealableBlock {
   ...
   public void performBonemeal(ServerLevel p_221270_, RandomSource p_221271_, BlockPos p_221272_, BlockState p_221273_) {
      ...
      Optional<Holder.Reference<PlacedFeature>> optional = p_221270_.registryAccess().registryOrThrow(Registries.PLACED_FEATURE).getHolder(RuVegetationPlacements.ALPHA_ROSE_BONEMEAL);
   ...
   public BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction action, boolean simulate) {
      if (ToolActions.SHOVEL_FLATTEN == action && context.getItemInHand().canPerformAction(ToolActions.SHOVEL_FLATTEN)) {
         ...
         if (block == RegionsUnexploredBlocks.ALPHA_GRASS_BLOCK.get()) {

...
public abstract class ChalkDirtBlock extends SnowyDirtBlock {
   ...
   public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
      if (!canBeGrass(state, level, pos)) {
         ...
         level.setBlockAndUpdate(pos, RegionsUnexploredBlocks.CHALK.get().defaultBlockState());
      } else {
         ...
         if (level.getMaxLocalRawBrightness(pos.above()) >= 9) {
            ...
            for(int i = 0; i < 4; ++i) {
               ...
               if (level.getBlockState(blockPos).is(RegionsUnexploredBlocks.CHALK.get()) && canPropagate(blockstate, level, blockPos)) {
               }
            }
         }

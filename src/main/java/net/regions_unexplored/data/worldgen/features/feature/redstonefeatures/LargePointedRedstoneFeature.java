...
public class LargePointedRedstoneFeature extends Feature<LargePointedRedstoneConfiguration> {
        ...
        if (!PointedRedstoneUtils.isEmptyOrWater(level, pos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (columnRange.height() < 4) {
                    ...
                    LargePointedRedstoneFeature.LargeRedstone largeRedstone = makeRedstone(pos.atY(columnRange.ceiling() - 1), false, random, k, largeRedstoneConfiguration.stalactiteBluntness, largeRedstoneConfiguration.heightScale);
                    LargePointedRedstoneFeature.LargeRedstone largeRedstone1 = makeRedstone(pos.atY(columnRange.floor() + 1), true, random, k, largeRedstoneConfiguration.stalagmiteBluntness, largeRedstoneConfiguration.heightScale);
                    LargePointedRedstoneFeature.WindOffsetter windOffsetter;
                    if (largeRedstone.isSuitableForWind(largeRedstoneConfiguration) && largeRedstone1.isSuitableForWind(largeRedstoneConfiguration)) {
                        windOffsetter = new LargePointedRedstoneFeature.WindOffsetter(pos.getY(), random, largeRedstoneConfiguration.windSpeed);
                    } else {
                        windOffsetter = LargePointedRedstoneFeature.WindOffsetter.noWind();
                    }
    ...
    static final class LargeRedstone {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel level, LargePointedRedstoneFeature.WindOffsetter windOffsetter) {
            while(this.radius > 1) {
                        ...
                        if (k > 0) {
                            ...
                            for(int i1 = 0; i1 < k && pos.getY() < l; ++i1) {
                                ...
                                if (PointedRedstoneUtils.isEmptyOrWaterOrLava(level, blockpos)) {
                                    flag = true;
                                    Block block = RegionsUnexploredBlocks.RAW_REDSTONE_BLOCK.get();
    ...
    static final class WindOffsetter {
        ...
        static LargePointedRedstoneFeature.WindOffsetter noWind() {
            return new LargePointedRedstoneFeature.WindOffsetter();
        }

...
public class HyacinthPlantsFeature extends Feature<ProbabilityFeatureConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<ProbabilityFeatureConfiguration> p_160318_) {
      ...
      BlockState blockstate = RegionsUnexploredBlocks.HYACINTH_SEAGRASS.get().defaultBlockState();
      ...
      if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER)) {
         if (blockstate.canSurvive(worldgenlevel, blockpos1)) {
            if(randomsource.nextInt(7)==0){
               blockstate = RegionsUnexploredBlocks.HYACINTH_BLOOM.get().defaultBlockState();
            }
            else{
               blockstate = RegionsUnexploredBlocks.HYACINTH_SEAGRASS.get().defaultBlockState();
            }

    ...
    private void placeRoofGlisteringWart(LevelAccessor level, RandomSource random, BlockPos pos) {
        ...
        for(int i = 0; i < 200; ++i) {
            ...
            if (level.isEmptyBlock(pos1)) {
                ...
                if (j == 1) {
                    level.setBlock(pos1, RegionsUnexploredBlocks.GLISTERING_WART.get().defaultBlockState(), 2);
                }
            }
        }
        ...
        for(int i = 0; i <= length; ++i) {
            if (level.isEmptyBlock(pos)) {
                if (i == length || !level.isEmptyBlock(pos.below())) {
                    level.setBlock(pos, RegionsUnexploredBlocks.GLISTERING_IVY.get().defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, low, high)), 2);
                    break;
                }

                level.setBlock(pos, RegionsUnexploredBlocks.GLISTERING_IVY_PLANT.get().defaultBlockState(), 2);
            }

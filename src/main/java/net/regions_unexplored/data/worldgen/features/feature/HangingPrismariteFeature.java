...
public class HangingPrismariteFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean placePillar(LevelAccessor level, BlockPos pos) {
        ...
        if (level.isEmptyBlock(pos) ) {
            if(level.getBlockState(pos.above()).isFaceSturdy(level, pos.above(), Direction.DOWN)
                    ||level.getBlockState(pos.above()).is(RegionsUnexploredBlocks.HANGING_PRISMARITE.get())) {
                ...
                while (level.isEmptyBlock(placePos)) {
                    ...
                    if(level.getBlockState(placePos.above()).is(RegionsUnexploredBlocks.HANGING_PRISMARITE.get())){
                        level.setBlock(placePos.above(), RegionsUnexploredBlocks.HANGING_PRISMARITE.get().defaultBlockState().setValue(HangingPrismariteBlock.SHAPE, HangingPrismariteShape.FRUSTUM), 2);
                    }
                    if(level.getBlockState(placePos.above().above()).is(RegionsUnexploredBlocks.HANGING_PRISMARITE.get())){
                        level.setBlock(placePos.above().above(), RegionsUnexploredBlocks.HANGING_PRISMARITE.get().defaultBlockState().setValue(HangingPrismariteBlock.SHAPE, HangingPrismariteShape.COLUMN), 2);
                    }
                    level.setBlock(placePos, RegionsUnexploredBlocks.HANGING_PRISMARITE.get().defaultBlockState(), 2);

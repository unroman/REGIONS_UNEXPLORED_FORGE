...
public class WaterCattailFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean placeCattail(LevelAccessor level, BlockPos pos) {
        if (level.getBlockState(pos.below()).is(RegionsUnexploredTags.CATTAIL_CAN_SURVIVE_ON)&&level.isWaterAt(pos)&&level.isEmptyBlock(pos.above())) {
            level.setBlock(pos, RegionsUnexploredBlocks.CATTAIL.get().defaultBlockState().setValue(CattailBlock.HALF, DoubleBlockHalf.LOWER).setValue(CattailBlock.WATERLOGGED, true), 2);
            level.setBlock(pos.above(), RegionsUnexploredBlocks.CATTAIL.get().defaultBlockState().setValue(CattailBlock.HALF, DoubleBlockHalf.UPPER).setValue(CattailBlock.WATERLOGGED, false), 2);
        }
        return true;
    }
}
...
public class HangingEarlightFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (!level.isEmptyBlock(pos)) {
            ...
            if (!blockstate.is(Blocks.BLACKSTONE) && !blockstate.is(RegionsUnexploredBlocks.COBALT_OBSIDIAN.get())) {
                return false;
            } else {
        ...
        for(int i = 0; i < 200; ++i) {
            ...
            if (level.isEmptyBlock(pos1)) {
                ...
                for(Direction direction : DIRECTIONS) {
                    ...
                    if (blockstate.is(Blocks.NETHERRACK) || blockstate.is(RegionsUnexploredBlocks.COBALT_OBSIDIAN.get())) {
                        ++j;
                    }
                ...
                if (j == 1) {
                    level.setBlock(pos1, RegionsUnexploredBlocks.COBALT_OBSIDIAN.get().defaultBlockState(), 2);
                }
            }
        }
        ...
        for(int i = 0; i <= length; ++i) {
            if (level.isEmptyBlock(pos)) {
                if (i == length || !level.isEmptyBlock(pos.below())) {
                    level.setBlock(pos, RegionsUnexploredBlocks.HANGING_EARLIGHT.get().defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, low, high)), 2);
                    break;
                }

                level.setBlock(pos, RegionsUnexploredBlocks.HANGING_EARLIGHT_PLANT.get().defaultBlockState(), 2);
            }
